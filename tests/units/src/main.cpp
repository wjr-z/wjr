...
// template class wjr::vector<int>;
// template class wjr::vector<std::string>;
...
TEST(vector, construct) {
    ...
    }
    {
        auto test = [](auto _Val, size_t n) {
            ...
            EXPECT_EQ(V.size(), n);
            EXPECT_GE(V.capacity(), n);
            std::for_each_n(v.data(), n, [&_Val](auto &x) { EXPECT_EQ(x, _Val); });
            std::for_each_n(V.data(), n, [&_Val](auto &x) { EXPECT_EQ(x, _Val); });
        };
        run_range([&](int i) {
            test(0, i);
...
TEST(vector, resize) {
    {
        auto test = [](auto _Val, size_t n, size_t s, size_t c) {
            ...
            EXPECT_EQ(v.size(), n);
            EXPECT_GE(v.capacity(), n);
            std::for_each_n(v.data(), s, [](auto &x) { EXPECT_EQ(x, T()); });
            if (n > s) {
                std::for_each_n(v.data() + s, n - s,
...
TEST(vector, reserve) {
    {
        auto test = [](auto _Val, size_t n, size_t s, size_t c) {
            ...
            EXPECT_EQ(v.size(), s);
            EXPECT_GE(v.capacity(), n);
            std::for_each_n(v.data(), s, [&_Val](auto &x) { EXPECT_EQ(x, _Val); });
        };
        run_range3([&](int n, int s, int c) {
            test(__int, n, s, c);
            test(__string, n, s, c);
...
TEST(vector, pop_back) {
    {
        auto test = [](auto _Val, size_t n) {
            ...
            EXPECT_GE(v.capacity(), n);
            std::for_each_n(v.data(), n - 1, [&_Val](auto &x) { EXPECT_EQ(x, _Val); });
        };
        run_range([&](int i) {
            test(__int, i);
            test(__string, i);
