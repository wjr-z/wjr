...
namespace wjr::math {
    ...
    WJR_CONSTEXPR unsigned char adc(unsigned char c_in, uint32_t* out,
        uint32_t in_1, uint32_t in_2) {
    ...
    WJR_CONSTEXPR unsigned char add(uint32_t* out, const uint32_t* in_1,
        const uint32_t* in_2, size_t n) {
        WASSERT_LEVEL_2(
            !((in_1 < out&& out < in_1 + n) || (in_2 < out&& out < in_2 + n)));
    ...
    WJR_CONSTEXPR unsigned char adc(unsigned char c_in, uint64_t* out,
        uint64_t in_1, uint64_t in_2) {
        WJR_IS_NOT_CONSTANT_EVALUATED_BEGIN
    ...
    __m512i adc512_u64(uint64_t& carry, __m512i A, __m512i B) {
        const __m512i MAX_WORD = _mm512_set1_epi64(0xffffffffffffffff);

        __m512i s = _mm512_add_epi64(A, B);
        __mmask8 c = _mm512_cmplt_epu64_mask(s, A);
        __mmask8 m = _mm512_cmpeq_epi64_mask(s, MAX_WORD);

        {
            uint8_t c0 = _cvtmask8_u32(c);
            uint8_t m0 = _cvtmask8_u32(m);
    ...
    WJR_CONSTEXPR unsigned char add(uint64_t* out, const uint64_t* in_1,
        const uint64_t* in_2, size_t n) {
        WASSERT_LEVEL_2(
            !((in_1 < out&& out < in_1 + n) || (in_2 < out&& out < in_2 + n)));
        ...
        for (; i != n; i += 8) {
            _mm512_storeu_si512(
                (__m512i*)(out + i),
                adc512_u64(cf, _mm512_loadu_si512((__m512i*)(in_1 + i)),
                    _mm512_loadu_si512((__m512i*)(in_2 + i))));
        }
        return cf;
#elif !WJR_HAS_BUILTIN(__builtin_addcll) && defined(WJR_GCC_STYLE_ASM) && \
    defined(WJR_X64)
            size_t r = n & 3;
        for (; i != r; ++i) {
            cf = adc(cf, out + i, in_1[i], in_2[i]);
    ...
    WJR_CONSTEXPR unsigned char sbb(unsigned char c_in, uint32_t* out,
        uint32_t in_1, uint32_t in_2) {
        WJR_IS_NOT_CONSTANT_EVALUATED_BEGIN
#if defined(_MSC_VER)
    ...
    WJR_CONSTEXPR unsigned char sub(uint32_t* out, const uint32_t* in_1,
        const uint32_t* in_2, size_t n) {
        WASSERT_LEVEL_2(
            !((in_1 < out&& out < in_1 + n) || (in_2 < out&& out < in_2 + n)));
    ...
    WJR_CONSTEXPR unsigned char sbb(unsigned char c_in, uint64_t* out,
        uint64_t in_1, uint64_t in_2) {
        WJR_IS_NOT_CONSTANT_EVALUATED_BEGIN
...
#if defined(__AVX512F__) && defined(__AVX512DQ__) && defined(__AVX512VL__)
    __m512i sbb512_u64(uint64_t& carry, __m512i A, __m512i B) {
        const __m512i MAX_WORD = _mm512_set1_epi64(0xffffffffffffffff);

        __m512i s = _mm512_sub_epi64(A, B);
        __mmask8 c = _mm512_cmpgt_epu64_mask(s, A);
        __mmask8 m = _mm512_cmpeq_epi64_mask(s, _mm512_setzero_si512());
    ...
    WJR_CONSTEXPR unsigned char sub(uint64_t* out, const uint64_t* in_1,
        const uint64_t* in_2, size_t n) {
        WASSERT_LEVEL_2(
            !((in_1 < out&& out < in_1 + n) || (in_2 < out&& out < in_2 + n)));
        ...
        for (; i != n; i += 8) {
            _mm512_storeu_si512(
                (__m512i*)(out + i),
                sbb512_u64(cf, _mm512_loadu_si512((__m512i*)(in_1 + i)),
                    _mm512_loadu_si512((__m512i*)(in_2 + i))));
        }
        return cf;
#elif !WJR_HAS_BUILTIN(__builtin_subcll) && defined(WJR_GCC_STYLE_ASM) && defined(WJR_X64)
            size_t r = n & 3;
        for (; i != r; ++i) {
            cf = sbb(cf, out + i, in_1[i], in_2[i]);
        }
    ...
    WJR_CONSTEXPR uint32_t vector_mul_1(uint32_t* result, const uint32_t* lp,
        size_t n, uint32_t rv) {
        uint32_t cf = 0;
        size_t r = n & 3;
        size_t i = 0;
        for (; i != r; ++i) {
    ...
    uint64_t vector_mul_1(uint64_t* result, const uint64_t* lp, size_t n,
        uint64_t rv) {
        uint64_t cf = 0;
        size_t r = n & 3;
        size_t i = 0;
    ...
    WJR_CONSTEXPR uint32_t vector_addmul_1(uint32_t* result, const uint32_t* lp,
        size_t n, uint32_t rv) {
        uint32_t cf = 0;
        size_t r = n & 3;
        size_t i = 0;
...
#if defined(__SIZEOF_INT128__)
    WJR_CONSTEXPR uint64_t vector_addmul_1(uint64_t* result, const uint64_t* lp,
        size_t n, uint64_t rv) {
        uint64_t cf = 0;
        size_t r = n & 3;
        size_t i = 0;
        for (; i != r; ++i) {
