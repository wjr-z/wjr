...
bool builtin_read_token_buffer(basic_lexer &lex) noexcept {
    ...
    auto idx = lex.idx;
    auto prev_is_escape = lex.prev_is_escape;
    auto prev_in_string = lex.prev_in_string;
    auto prev_is_ws = lex.prev_is_ws;
    auto token_last = lex.token_buf;
    ...
    do {
        ...
        idx += 64;
    } while (WJR_LIKELY(count <= 64));

    lex.first = first;
    lex.idx = idx;
    lex.prev_is_escape = prev_is_escape;
    lex.prev_in_string = prev_in_string;
    lex.prev_is_ws = prev_is_ws;
    lex.token_last = token_last;
