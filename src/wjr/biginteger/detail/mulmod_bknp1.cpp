...
static void _mpn_modbknp1dbnp1_n(mp_ptr rp, mp_srcptr op, mp_size_t n, unsigned k) {
    mp_limb_t hl;
    mp_srcptr hp;
    unsigned i;
...
static void _mpn_modbnp1_pn_ip(mp_ptr r, mp_size_t n, mp_limb_t h) {
    ASSERT(r[n] == h);
...
static void _mpn_modbnp1_neg_ip(mp_ptr r, mp_size_t n, mp_limb_t h) {
    r[n] = 0;
    MPN_INCR_U(r, n + 1, -h);
    if (UNLIKELY(r[n] != 0))
...
static void _mpn_modbnp1_nc_ip(mp_ptr r, mp_size_t n, mp_limb_t h) {
...
static void _mpn_modbnp1(mp_ptr rp, mp_size_t rn, mp_srcptr op, mp_size_t on) {
    mp_limb_t bw;
...
/* With odd k >= 3. */
static void _mpn_modbnp1_kn(mp_ptr rp, mp_srcptr op, mp_size_t rn, unsigned k) {
    mp_limb_t cy;
...
/* For the various mpn_divexact_byN here, fall back to using either
   ...
   mpn_pi1_bdiv_q_1 unconditionally.  FIXME.  */
    ...
    #if HAVE_NATIVE_mpn_pi1_bdiv_q_1
        #define BINVERT_11 ((((GMP_NUMB_MAX >> (GMP_NUMB_BITS % 10)) / 11 << 5) + 3) & GMP_NUMB_MAX)
        #define mpn_divexact_by11(dst, src, size)                                                  \
            mpn_pi1_bdiv_q_1(dst, src, size, 11, BINVERT_11, 0)
    #else
        ...
        #define mpn_divexact_by13(dst, src, size)                                                  \
            mpn_pi1_bdiv_q_1(dst, src, size, 13, BINVERT_13, 0)
    #else
        ...
        #define mpn_divexact_by17(dst, src, size)                                                  \
            mpn_pi1_bdiv_q_1(dst, src, size, 17, BINVERT_17, 0)
    #else
...
*/
static void _mpn_crt(mp_ptr rp, mp_srcptr ap, mp_srcptr bp, mp_size_t n, unsigned k, mp_ptr tp) {
    mp_limb_t mod;
    unsigned i;
    ...
    else if (((!MOD_BKNP1_USE11) && (GMP_NUMB_BITS % 16 != 0)) || LIKELY(k == 7))
...
static void _mpn_mulmod_bnp1_tp(mp_ptr rp, mp_srcptr ap, mp_srcptr bp, mp_size_t rn, mp_ptr tp) {
    mp_limb_t cy;
    unsigned k;
...
static void _mpn_sqrmod_bnp1_tp(mp_ptr rp, mp_srcptr ap, mp_size_t rn, mp_ptr tp) {
    mp_limb_t cy;
    unsigned k;
...
void mulmod_bknp1(uint64_t *rp, const uint64_t *ap, const uint64_t *bp, size_t n, unsigned k,
...
void sqrmod_bknp1(uint64_t *rp, const uint64_t *ap, size_t n, unsigned k, uint64_t *tp) noexcept {
    mp_ptr hp;
