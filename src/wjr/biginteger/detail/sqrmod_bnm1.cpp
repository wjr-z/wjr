...
static void mpn_bc_sqrmod_bnp1(mp_ptr rp, mp_srcptr ap, mp_size_t rn, mp_ptr tp) {
    ...
    if (UNLIKELY(ap[rn])) {
        *rp = 1;
        ...
        TMP_MARK;
        mpn_sqrmod_bknp1(rp, ap, n_k, k, TMP_ALLOC_LIMBS(mpn_sqrmod_bknp1_itch(rn)));
        TMP_FREE;
        return;
    }
    mpn_sqr(tp, ap, rn);
 ...
 */
void sqrmod_bnm1(uint64_t *rp, size_t rn, const uint64_t *ap, size_t an, uint64_t *tp) noexcept {
    ASSERT(0 < an);
    ASSERT(an <= rn);
       ...
       ([(xp + xm)/2 mod (B^n-1)] - xp ) * B^n
        ...
        if (UNLIKELY(2 * an < rn)) {
            ...
            /* FIXME: This subtraction of the high parts is not really
               ...
               checking. */
            cy = xp[n] +
                 mpn_sub_nc(xp + 2 * an - n, rp + 2 * an - n, xp + 2 * an - n, rn - 2 * an, cy);
