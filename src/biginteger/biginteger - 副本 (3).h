...
#pragma GCC unroll(4)
            ...
            struct __unsigned_biginteger_modifier {
                ...
                constexpr decltype(auto) operator()(unsigned_biginteger<base>& val, size_t _Offset, size_t _Size) {
                    return unsigned_biginteger_modifier<base>(val, _Offset, _Size);
                }
            };
...
#define UNSIGNED_BIGINTEGER_VIEW_BINARY_AND_TAG_DECLARATION                                     \
	template<typename T, typename U, typename C, std::enable_if_t<                              \
            std::conjunction_v<__Can_make_unsigned_view<base, std::decay_t<T>>,                 \
            ...
            public:
                ...
                WJR_CONSTEXPR20 void maintain(size_t _Offset) {
                    WASSERT_LEVEL_1(_Offset < _Base::size());
                    size_t n = _Base::size();
                    auto p = _Base::data();
                    while (n != _Offset + 1 && !p[n - 1])--n;
                    _Base::set_size(n);
                }
