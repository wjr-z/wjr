#ifndef WJR_CONCURRENCY_RDTSC_HPP__
#define WJR_CONCURRENCY_RDTSC_HPP__
...
#if defined(WJR_COMPILER_MSVC) && !defined(_M_IX86) && !defined(_M_ARM64) && !defined(_M_ARM64EC)
extern "C" uint64_t __rdtsc();
    #pragma intrinsic(__rdtsc)
#endif

#if !defined(WJR_OS_WINDOWS) || defined(WJR_OS_MINGW)
    #include <sys/time.h>
    #include <time.h>
#endif
...
WJR_INTRINSIC_INLINE int64_t rdtsc() noexcept {
    ...
    return itc;
#elif defined(WJR_COMPILER_MSVC) && defined(_M_IX86)
    // Older MSVC compilers (like 7.x) don't seem to support the
    // __rdtsc intrinsic properly, so I prefer to use _asm instead
    // when I know it will work.  Otherwise, I'll use __rdtsc and hope
    // the code is being compiled with a non-ancient compiler.
    _asm rdtsc
#elif defined(WJR_COMPILER_MSVC) && (defined(_M_ARM64) || defined(_M_ARM64EC))
    ...
    uint64_t tsc;
    #if defined(WJR_OS_ZOS) && defined(COMPILER_IBMXL)
...
#endif // WJR_CONCURRENCY_RDTSC_HPP__