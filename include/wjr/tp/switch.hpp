...
default: {                                                                               \
    if constexpr (std::is_invocable_v<Func &&, switch_default_t, Args &&...>) {          \
        using Ret =                                                                      \
            std::invoke_result_t<Func &&, integral_constant<size_t, 0>, Args &&...>;     \
        using DRet = std::invoke_result_t<Func &&, switch_default_t, Args &&...>;        \
        if constexpr (std::is_same_v<Ret, DRet>) {                                       \
            return std::forward<Func>(func)(switch_default,                              \
                                            std::forward<Args>(args)...);                \
        } else {                                                                         \
...
#define WJR_REGISTER_SWITCH_VISITOR(SZ)                                                  \
    ...
    struct siwtch_visitor_helper<SZ, C, Size> {                                          \
        ...
        using CaseList =                                                                 \
            tp_concat_t<C, tp_exclude_t<tp_iota_t<value_type, MinValue, SZ>, C>>;        \
                                                                                         \
        template <typename Val, typename Func, typename... Args>                         \
        WJR_INTRINSIC_CONSTEXPR static decltype(auto) visit(Val ms, Func &&func,         \
                                                            Args &&...args) {            \
            switch (ms) {                                                                \
...
WJR_INTRINSIC_CONSTEXPR decltype(auto) vswitch(Val ms, Func &&func, Args &&...args) {
    return switch_visitor_t<C>::visit(ms, std::forward<Func>(func),
                                      std::forward<Args>(args)...);
}
...
template <char... Chars>
using __tp_string_list_t =
    tp_rename_t<tp_integers_list_t<char, Chars...>, __tp_string_list>;

template <typename... Args0, typename... Args1>
struct tp_less<__tp_string_list<Args0...>, __tp_string_list<Args1...>>
    : tp_lexicographical_compare_t<__tp_string_list<Args0...>,
                                   __tp_string_list<Args1...>> {};
