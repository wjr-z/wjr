...
__m128i sse::alignr(__m128i a, __m128i b) noexcept {
    ...
    if constexpr (s < 16) {
        if constexpr (s == 8) {
            return simd_cast<__m128i_t, __m128_t>(
                shuffle_ps<_MM_SHUFFLE(1, 0, 3, 2)>((__m128)b, (__m128)a));
        } else {
...
uint64_t sse::getlow(__m128i v) noexcept { return simd_cast<__m128i_t, uint64_t>(v); }
