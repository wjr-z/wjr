...
// TODO : Temporarily discarded. Waiting for fix.
 ...
 * 8.static void Swap(_Al& al, vector_data& _Left,
 *  vector_data& _Right) [noexcept] [optional]
 * 9.static void Deallocate(_Al& al, vector_data& _Data)
 * 10.static size_t getGrowthCapacity(const size_t _Oldcapacity,
 * const size_t _Newsize) [optinal]
...
struct vector_data {
    ...
    WJR_CONSTEXPR20 vector_data(_Alty &al, const size_type _Newsize,
                                const size_type _Newcapacity)
        : _Myfirst(_Alty_traits::allocate(al, _Newcapacity)),
          _Mylast(_Myfirst + _Newsize), _Myend(_Myfirst + _Newcapacity) {}

    WJR_CONSTEXPR20 vector_data(_Alty &al, const size_type _Newsize,
                                const size_type _Newcapacity, extend_t)
        : vector_data(al, _Newsize, _Newcapacity) {}

    WJR_CONSTEXPR20 static void copyConstruct(_Alty &al, const vector_data &_Src,
                                              vector_data &_Dest) {
        {
    ...
    WJR_INLINE_CONSTEXPR20 static void moveConstruct(_Alty &, vector_data &&_Src,
                                                     vector_data &_Dest) noexcept {
    ...
    WJR_ATTRIBUTE(PURE, INLINE_CONSTEXPR20) pointer data() noexcept { return _Myfirst; }

    WJR_ATTRIBUTE(PURE, INLINE_CONSTEXPR20) const_pointer data() const noexcept {
        return _Myfirst;
    }

    WJR_ATTRIBUTE(PURE, INLINE_CONSTEXPR20) pointer lastPtr() noexcept { return _Mylast; }

    WJR_ATTRIBUTE(PURE, INLINE_CONSTEXPR20) const_pointer lastPtr() const noexcept {
        return _Mylast;
    }

    WJR_ATTRIBUTE(PURE, INLINE_CONSTEXPR20) pointer endPtr() noexcept { return _Myend; }
...
struct vector_static_data {
    ...
    constexpr static size_t _MaxMemroy =
    ...
    _lengthError(WJR_MAYBE_UNUSED const size_type _Newcapacity) {
#if defined(_WJR_EXCEPTION)
        throw std::length_error(
            "vector_static_data is too small to hold the requested data");
#endif // _WJR_EXCEPTION
        WJR_UNREACHABLE;
    }

    WJR_CONSTEXPR20 vector_static_data(_Alty &, const size_type,
                                       const size_type _Newcapacity, extend_t) {
        _lengthError(_Newcapacity);
    }

    WJR_CONSTEXPR20 vector_static_data(_Alty &, const size_type _Newsize,
                                       const size_type _Newcapacity)
        : _M_size(_Newsize) {
    ...
    WJR_CONSTEXPR20 static void copyConstruct(_Alty &al, const vector_static_data &_Src,
                                              vector_static_data &_Dest) {
        wjr::uninitialized_copy_n(al, _Src.data(), _Src.size(), _Dest.data());
        _Dest.set_size(_Src.size());
    }

    WJR_CONSTEXPR20 static void moveConstruct(_Alty &al, vector_static_data &&_Src,
                                              vector_static_data &_Dest) {
        auto n = _Src.size();
        if (n != 0) {
    ...
    WJR_INLINE_CONSTEXPR20 static void Deallocate(_Alty &,
                                                  vector_static_data &_Data) noexcept {
        _Data.set_size(0);
    }
...
struct vector_sso_data {
    ...
    constexpr static size_t _MaxAlignment =
        std::max(alignof(T), std::max(alignof(size_type), alignof(pointer)));
    constexpr static size_t _MaxMemroy = std::max(
        (sizeof(T) * N + _MaxAlignment - 1) & (~(_MaxAlignment - 1)), sizeof(size_type));
    constexpr static size_t max_capacity = _MaxMemroy / sizeof(T);
    static_assert(max_capacity >= N, "");
    ...
    WJR_CONSTEXPR20 vector_sso_data(_Alty &al, const size_type _Newsize,
                                    const size_type _Newcapacity, extend_t)
        : _M_ptr(_Alty_traits::allocate(al, _Newcapacity)), _M_size(_Newsize),
    ...
    WJR_CONSTEXPR20 static void copyConstruct(_Alty &al, const vector_sso_data &_Src,
                                              vector_sso_data &_Dest) {
    ...
    WJR_INLINE_CONSTEXPR20 static void moveConstruct(_Alty &al, vector_sso_data &&_Src,
                                                     vector_sso_data &_Dest) {
        const auto _Size = _Src._M_size;
        if (!_Src.is_small()) {
        ...
        if (!_Data.is_small()) {
            ...
            if (_Oldsize != _Oldcapacity) {
                if (_Oldsize <= max_capacity) {
                    wjr::uninitialized_move_n(
                        al, _Data.data(), _Oldsize,
                        reinterpret_cast<pointer>(_Data._M_storage));
    ...
    WJR_ATTRIBUTE(PURE, INLINE_CONSTEXPR20) pointer data() noexcept { return _M_ptr; }
    ...
    WJR_INLINE_CONSTEXPR20 bool is_small() const noexcept {
        const bool f =
            static_cast<const void *>(_M_ptr) == static_cast<const void *>(_M_storage);
        return f;
    }
...
template <typename T, typename Alloc = std::allocator<T>,
          typename Data = vector_data<T, Alloc>>
class vector;
...
public:
    ...
    WJR_CONSTEXPR20
    vector() noexcept(std::is_nothrow_default_constructible_v<_Alty>
    ...
    WJR_CONSTEXPR20 explicit vector(const allocator_type &al) noexcept(
        std::is_nothrow_constructible_v<_Alty, const allocator_type &>
            &&std::is_nothrow_default_constructible_v<data_type>)
        : _Myval(std::piecewise_construct, std::forward_as_tuple(al),
                 std::forward_as_tuple()) {}

    WJR_CONSTEXPR20 explicit vector(const size_type _Count,
                                    const allocator_type &al = allocator_type())
        : vector(al) {
        _M_construct_n(_Count, value_construct_t());
    }

    WJR_CONSTEXPR20 vector(size_type _Count, const value_type &_Val,
                           const allocator_type &al = allocator_type())
        : vector(al) {
...
private:
    constexpr static bool _IsNoThrowCopyConstruct = noexcept(data_type::copyConstruct(
        std::declval<_Alty &>(), std::declval<const data_type &>(),
        std::declval<data_type &>()));
    constexpr static bool _IsNoThrowMoveConstruct = noexcept(
        data_type::moveConstruct(std::declval<_Alty &>(), std::declval<data_type &&>(),
                                 std::declval<data_type &>()));

public:
    template <typename _Alloc>
    WJR_CONSTEXPR20 vector(const vector &other, _Alloc &&al, disable_t) noexcept(
        _IsNoThrowCopyConstruct &&std::is_nothrow_constructible_v<_Alty, _Alloc &&>)
    ...
    WJR_CONSTEXPR20 vector(const vector &other)
        : vector(
              other,
              _Alty_traits::select_on_container_copy_construction(other.getAllocator()),
              disable_t{}) {}

    WJR_CONSTEXPR20 vector(const vector &other, const allocator_type &al)
        : vector(other, al, disable_t{}) {}

    WJR_CONSTEXPR20 vector(vector &&other) noexcept(
        _IsNoThrowMoveConstruct &&std::is_nothrow_move_constructible_v<_Alty>)
        : _Myval(std::piecewise_construct,
...
private:
    WJR_INTRINSIC_CONSTEXPR20 static void do_move_construct(vector &a,
                                                            vector &&b) noexcept {
        moveConstruct(a.getAllocator(), std::move(b.getData()), a.getData());
    }

    WJR_INTRINSIC_CONSTEXPR20 static void
    do_move_construct_by_element(vector &a, vector &&b) noexcept {
    ...
    WJR_INTRINSIC_CONSTEXPR20 static void do_move_assign_by_element(vector &a,
                                                                    vector &&b) noexcept {
...
public:
    WJR_CONSTEXPR20 vector(vector &&other, const allocator_type &al) noexcept
        : _Myval(std::piecewise_construct, std::forward_as_tuple(al),
                 std::forward_as_tuple()) {
    ...
    WJR_CONSTEXPR20 vector(_Iter _First, _Iter _Last,
                           const allocator_type &al = allocator_type())
        : vector(al) {
        _M_range_init(_First, _Last,
                      typename std::iterator_traits<_Iter>::iterator_category());
    }

    WJR_CONSTEXPR20 vector(std::initializer_list<value_type> il,
                           const allocator_type &al = allocator_type())
        : vector(il.begin(), il.end(), al) {}

    WJR_CONSTEXPR20 ~vector() noexcept { tidy(); }
    ...
    WJR_CONSTEXPR20 vector &operator=(vector &&other) noexcept(
    ...
    WJR_INLINE_CONSTEXPR20 iterator end() noexcept { return lastPtr(); }

    WJR_INLINE_CONSTEXPR20 const_iterator end() const noexcept { return lastPtr(); }

    WJR_INLINE_CONSTEXPR20 const_iterator cend() const noexcept { return lastPtr(); }
    ...
    WJR_CONSTEXPR20 void shrink_to_fit() { shrinkToFit(getAllocator(), getData()); }
    ...
    WJR_INLINE_CONSTEXPR20 reference back() noexcept { return *(lastPtr() - 1); }
    ...
    WJR_INLINE_CONSTEXPR20 void push_back(const value_type &_Val) { emplace_back(_Val); }
    ...
    WJR_INLINE_CONSTEXPR20 iterator insert(const_iterator _Where,
    ...
    WJR_CONSTEXPR20 iterator insert(const_iterator _Where,
                                    std::initializer_list<value_type> il) {
        return insert(_Where, il.begin(), il.end());
    }

    WJR_CONSTEXPR20 iterator insert(const_iterator _Where, size_type _Count,
                                    const value_type &_Val) {
    ...
    WJR_CONSTEXPR20 iterator insert(const_iterator _Where, iter _First, iter _Last) {
        const auto __old_pos = static_cast<size_type>(_Where - cbegin());
        _M_range_insert(begin() + __old_pos, _First, _Last,
                        typename std::iterator_traits<iter>::iterator_category());
        return begin() + __old_pos;
    }
    ...
    WJR_CONSTEXPR20 static void
    copyConstruct(_Alty &al, const data_type &_Src,
                  data_type &_Dest) noexcept(_IsNoThrowCopyConstruct) {
        data_type::copyConstruct(al, _Src, _Dest);
    }

    WJR_INLINE_CONSTEXPR20 static void
    moveConstruct(_Alty &al, data_type &&_Src,
                  data_type &_Dest) noexcept(_IsNoThrowMoveConstruct) {
        data_type::moveConstruct(al, std::move(_Src), _Dest);
    }

    template <typename _D = data_type,
              std::enable_if_t<_Vector_helper::has_static_member_function_Swap_v<
                                   _D, _Alty &, _D &, _D &>,
                               int> = 0>
    WJR_INLINE_CONSTEXPR20 static void
    Swap(_Alty &al, _D &_Left,
         _D &_Right) noexcept(noexcept(_D::Swap(al, _Left, _Right))) {
        return _D::Swap(al, _Left, _Right);
    }

    template <typename _D = Data,
              std::enable_if_t<!_Vector_helper::has_static_member_function_Swap_v<
                                   _D, _Alty &, _D &, _D &>,
                               int> = 0>
    WJR_INLINE_CONSTEXPR20 static void Swap(_Alty &al, _D &_Left, _D &_Right) noexcept(
        noexcept(moveConstruct(std::declval<_Alty &>(), std::declval<_D &&>(),
                               std::declval<_D &>())) &&
    ...
    WJR_CONSTEXPR20 static void
    Deallocate(_Alty &al,
               data_type &_Data) noexcept(noexcept(data_type::Deallocate(al, _Data))) {
    ...
    template <
        typename _D = Data,
        std::enable_if_t<_Vector_helper::has_static_member_function_getGrowthCapacity_v<
                             _D, const size_type, const size_type>,
                         int> = 0>
    WJR_INLINE_CONSTEXPR20 static size_type
    getGrowthCapacity(const size_type _Oldcapacity, const size_type _Newsize) noexcept {
    ...
    WJR_INLINE_CONSTEXPR20 data_type &getData() noexcept { return _Myval.second(); }
    ...
    WJR_CONSTEXPR20 vector(const size_type _Count, default_construct_t,
                           const allocator_type &al = allocator_type())
        : vector(al) {
        _M_construct_n(_Count, default_construct_t());
    }

    WJR_CONSTEXPR20 vector(const size_type _Count, value_construct_t,
                           const allocator_type &al = allocator_type())
        : vector(al) {
        _M_construct_n(_Count, value_construct_t());
    }
    ...
    template <typename iter, std::enable_if_t<is_iterator_v<iter>, int> = 0>
    WJR_CONSTEXPR20 vector &replace(const_iterator _Oldfirst, const_iterator _Oldlast,
        ...
        _M_range_replace(__beg + __offset1, __beg + __offset2, _Newfirst, _Newlast,
                         typename std::iterator_traits<iter>::iterator_category());
        return *this;
    }
    ...
    WJR_CONSTEXPR20 vector &replace(const_iterator _Oldfirst, const_iterator _Oldlast,
        ...
        const auto __offset1 = static_cast<size_type>(_Oldfirst - __cbeg);
        const auto __offset2 = static_cast<size_type>(_Oldlast - __cbeg);
        _M_fill_replace(__beg + __offset1, __beg + __offset2, _Count, _Val);
        return *this;
    }
...
private:
    template <typename... Args,
              std::enable_if_t<is_any_index_of_v<sizeof...(Args), 1, 2>, int> = 0>
    ...
    template <typename iter>
    WJR_CONSTEXPR20 void _M_range_init(iter _First, iter _Last,
    ...
    template <typename iter>
    WJR_CONSTEXPR20 void _M_range_insert(iterator _Where, iter _First, iter _Last,
        ...
        if (_First != _Last) {
            ...
            if (__rest >= n) {
                ...
                if (__elements_after > n) {
                    ...
                    wjr::uninitialized_move(al, _Where, __old_last,
                                            _Mylast + n - __elements_after);
                ...
                const size_type _Newcapacity =
                    getGrowthCapacity(capacity(), __old_size + n);
    ...
    template <typename iter>
    WJR_CONSTEXPR20 void _M_range_append(iter _First, iter _Last,
                                         std::input_iterator_tag) {
        for (; _First != _Last; ++_First) {
            emplace_back(*_First);
        }
    ...
    template <typename iter>
    WJR_CONSTEXPR20 void _M_range_assign(iter _First, iter _Last,
                                         std::input_iterator_tag) {
        const pointer _Myfirst = data();
        const pointer _Mylast = lastPtr();
    ...
    WJR_CONSTEXPR20 void _M_move_assign(vector &&other, std::false_type) {
        if (getAllocator() == other.getAllocator()) {
            ...
            _M_range_assign(std::make_move_iterator(other.begin()),
    ...
    WJR_CONSTEXPR20 void _M_fill_insert(iterator _Where, size_type n,
                                        const value_type &_Val) {
        if (n == 0) {
            return;
        }
    ...
    template <typename iter>
    WJR_CONSTEXPR20 void _M_range_replace(iterator _Oldfirst, iterator _Oldlast,
                                          iter _Newfirst, iter _Newlast,
                                          std::input_iterator_tag) {
    ...
    WJR_CONSTEXPR20 void _M_range_replace(iterator _Oldfirst, iterator _Oldlast,
        ...
        if (__m <= __n) {
            ...
            if (__rest >= __delta) {
                ...
                const auto _Newcapacity =
                    getGrowthCapacity(capacity(), __old_size + __delta);
    ...
    WJR_CONSTEXPR20 void _M_fill_replace(iterator _Oldfirst, iterator _Oldlast,
...
swap(wjr::vector<T, Alloc, Data> &lhs,
     wjr::vector<T, Alloc, Data> &rhs) noexcept(noexcept(lhs.swap(rhs))) {
    lhs.swap(rhs);
}

template <typename T, typename Alloc, typename Data>
WJR_CONSTEXPR20 typename wjr::vector<T, Alloc, Data>::size_type
erase(wjr::vector<T, Alloc, Data> &c, const T &value) {
...
WJR_CONSTEXPR20 typename wjr::vector<T, Alloc, Data>::size_type
erase_if(wjr::vector<T, Alloc, Data> &c, Pred pred) {
