...
WJR_INTRINSIC_CONSTEXPR20 U subc_1(uint64_t *dst, const uint64_t *src0, size_t n, uint64_t src1,
                                   U c_in) noexcept {
...
template <typename U>
WJR_INTRINSIC_CONSTEXPR U fallback_subc_n(uint64_t *dst, const uint64_t *src0, const uint64_t *src1,
...
*/
template <typename U, WJR_REQUIRES_I(is_unsigned_integral_v<U>)>
WJR_INTRINSIC_CONSTEXPR20 U subc_n(uint64_t *dst, const uint64_t *src0, const uint64_t *src1,
                                   size_t n, U c_in) noexcept {
    WJR_ASSERT_ASSUME(n >= 1);
    WJR_ASSERT_L2(WJR_IS_SAME_OR_INCR_P(dst, n, src0, n));
...
WJR_INTRINSIC_INLINE void __builtin_sub_128(uint64_t &al, uint64_t &ah, uint64_t lo0, uint64_t hi0,
