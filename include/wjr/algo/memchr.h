...
const T* __memchr(const T* s, T val, size_t n) {
	...
	if (n >= 16 / _Mysize) {
		if (n >= width * 4) {
			...
			if constexpr (_Mysize == 1) {
				...
				do {
					...
					r3 = simd_t::Or(simd_t::Or(r0, r1), simd_t::Or(r2, r3));

					simd_t::mask_type z = simd_t::movemask_epi8(r3);
					if (z != 0) {
						simd_t::mask_type tmp = simd_t::movemask_epi8(r0);
						if (tmp) {
							return s + wjr::countr_zero(tmp);
						}
						tmp = simd_t::movemask_epi8(r1);
						if (tmp) {
							return s + width + wjr::countr_zero(tmp);
						}
						tmp = simd_t::movemask_epi8(r2);
						if (tmp) {
							return s + width * 2 + wjr::countr_zero(tmp);
						}
						tmp = z;
						return s + width * 3 + wjr::countr_zero(tmp);
					}
					s += width * 4;
					n -= width * 4;
				} while (n >= width * 4);
				...
				if (n != 0) {
					...
					case 1: {
						simd_t::mask_type z = simd_t::movemask_epi8(simd_t::cmpeq(
							simd_t::loadu(reinterpret_cast<const sint*>(_last - width)),
							y, T()));
						if (z != 0) {
							return _last - width + wjr::countr_zero(z);
						}
					}
					}
				}
			...
			// unalign algorithm
				...
				do {
					...
					if (z != 0) {
						simd_t::mask_type tmp = simd_t::movemask_epi8(r0);
						if (tmp) {
							return s + wjr::countr_zero(tmp) / _Mysize;
						}
						tmp = simd_t::movemask_epi8(r1);
						if (tmp) {
							return s + width + wjr::countr_zero(tmp) / _Mysize;
						}
						tmp = simd_t::movemask_epi8(r2);
						if (tmp) {
							return s + width * 2 + wjr::countr_zero(tmp) / _Mysize;
						}
						tmp = z;
						return s + width * 3 + wjr::countr_zero(tmp) / _Mysize;
					}
					s += width * 4;
					n -= width * 4;
				} while (n >= width * 4);
				...
				if (n != 0) {
					...
					case 4: {
						simd_t::mask_type z = simd_t::movemask_epi8(simd_t::cmpeq(
							...
							y, T()));
						if (z != 0) {
							return s + wjr::countr_zero(z) / _Mysize;
						}
						s += width;
					}
					case 3: {
					...
					case 1: {
						simd_t::mask_type z = simd_t::movemask_epi8(simd_t::cmpeq(
							...
							y, T()));
						if (z != 0) {
							return _last - width + wjr::countr_zero(z) / _Mysize;
						}
					}
					}
				}
		...
		if (n >= 64 / _Mysize) {
			...
			auto r0 = simd::avx::cmpeq(simd::avx::loadu(reinterpret_cast<const __m256i*>(s)), y, T());
			auto r1 = simd::avx::cmpeq(simd::avx::loadu(reinterpret_cast<const __m256i*>(s + 32 / _Mysize)), y, T());
			auto r2 = simd::avx::cmpeq(simd::avx::loadu(reinterpret_cast<const __m256i*>(s + n - (64 / _Mysize))), y, T());
			auto r3 = simd::avx::cmpeq(simd::avx::loadu(reinterpret_cast<const __m256i*>(s + n - (32 / _Mysize))), y, T());

			r3 = simd::avx::Or(simd::avx::Or(r0, r1), simd::avx::Or(r2, r3));

			uint32_t z = simd::avx::movemask_epi8(r3);
			if (z != 0) {
				uint32_t tmp = simd::avx::movemask_epi8(r0);
				if (tmp) {
					return s + wjr::countr_zero(tmp) / _Mysize;
				}
				tmp = simd::avx::movemask_epi8(r1);
				if (tmp) {
					return s + (32 / _Mysize) + wjr::countr_zero(tmp) / _Mysize;
				}
				tmp = simd::avx::movemask_epi8(r2);
				if (tmp) {
					return s + n - (64 / _Mysize) + wjr::countr_zero(tmp) / _Mysize;
				}
				tmp = z;
				return s + n - (32 / _Mysize) + wjr::countr_zero(tmp) / _Mysize;
			}
			return s + n;
		}
#endif // __AVX2__
		...
		auto r0 = simd::sse::cmpeq(simd::sse::loadu(reinterpret_cast<const __m128i*>(s)), y, T());
		auto r1 = simd::sse::cmpeq(simd::sse::loadu(reinterpret_cast<const __m128i*>(s + delta)), y, T());
		auto r2 = simd::sse::cmpeq(simd::sse::loadu(reinterpret_cast<const __m128i*>(s + n - (16 / _Mysize) - delta)), y, T());
		auto r3 = simd::sse::cmpeq(simd::sse::loadu(reinterpret_cast<const __m128i*>(s + n - (16 / _Mysize))), y, T());

		r3 = simd::sse::Or(simd::sse::Or(r0, r1), simd::sse::Or(r2, r3));

		uint16_t z = simd::sse::movemask_epi8(r3);
		if (z != 0) {
			uint16_t tmp = simd::sse::movemask_epi8(r0);
			if (tmp) {
				return s + wjr::countr_zero(tmp) / _Mysize;
			}
			tmp = simd::sse::movemask_epi8(r1);
			if (tmp) {
				return s + delta + wjr::countr_zero(tmp) / _Mysize;
			}
			tmp = simd::sse::movemask_epi8(r2);
			if (tmp) {
				return s + n - (16 / _Mysize) - delta + wjr::countr_zero(tmp) / _Mysize;
			}
			tmp = z;
			return s + n - (16 / _Mysize) + wjr::countr_zero(tmp) / _Mysize;
		}
		return s + n;
	}
