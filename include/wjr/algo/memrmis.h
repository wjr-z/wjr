...
#define __REGISTER_MEMRMIS_FUNC_FOUR(s0, s1)															    \
	{																						                \
		...
		auto Z = traits::movemask_epi8(traits::And(traits::And(r0, r1), traits::And(r2, r3)));			    \
																							                \
		if(Z != __Mask){																	                \
			auto z = traits::movemask_epi8(r3);											                    \
			if(z != __Mask){																                \
				return (s0) + _Mysize * 4 - wjr::countl_one(static_cast<value_type>(z)) / _Mycor;	        \
			}																				                \
			z = traits::movemask_epi8(r2);											                        \
			if(z != __Mask){																                \
				return (s0) + _Mysize * 3 - wjr::countl_one(static_cast<value_type>(z)) / _Mycor;	        \
			}																				                \
			z = traits::movemask_epi8(r1);											                        \
			if(z != __Mask){																                \
				return (s0) + _Mysize * 2 - wjr::countl_one(static_cast<value_type>(z)) / _Mycor;	        \
			}																				                \
			z = traits::movemask_epi8(r0);											                        \
			return (s0) + _Mysize * 1 - wjr::countl_one(static_cast<value_type>(z)) / _Mycor;	            \
		}																					                \
	}
...
const T* __memrmis(const T* s0, const T* s1, size_t n, _Pred pred) {
	...
	if (n < _Mysize) {
		...
		if (ql) {
			s0 -= sl / sizeof(T);
			s1 -= sl / sizeof(T);
			auto x0 = traits::preloadu_si16x(s0, ql);
			auto x1 = traits::preloadu_si16x(s1, ql);
