#ifndef WJR_ALLOCATOR_HPP__
#define WJR_ALLOCATOR_HPP__
...
public:
    ...
    // Allocate memory
    value_type *allocate(size_type n) const { return get_instance().allocate(n); }

    // Deallocate memory
    void deallocate(value_type *p, size_type n) const { get_instance().deallocate(p, n); }
};
