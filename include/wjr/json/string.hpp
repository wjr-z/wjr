...
WJR_INTRINSIC_INLINE result<const char *> parse_unicode_codepoint(char *&dst, const char *first,
                                                                  const char *last) noexcept {
    if (const auto ret = utf8::parse_unicode_codepoint(reinterpret_cast<uint8_t *&>(dst),
...
WJR_INTRINSIC_INLINE result<const char *> check_unicode_codepoint(const char *first,
                                                                  const char *last) noexcept {
    if (const auto ret = utf8::check_unicode_codepoint(reinterpret_cast<const uint8_t *>(first),
                                                       reinterpret_cast<const uint8_t *>(last));
        WJR_UNLIKELY(!ret)) {
        return unexpected(error_code::UTF8_ERROR);
    } else {
...
WJR_PURE WJR_INTRINSIC_INLINE result<void> generic_check_string(const char *first,
                                                                const char *last) noexcept {
    using namespace string_detail;
