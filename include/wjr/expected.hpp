...
public:
    ...
    friend constexpr bool operator==(unexpected &x,
                                     unexpected<E2> &y) noexcept(has_noexcept_equal_to_v<E, E2>) {
        return x.error() == y.error();
    }

    template <typename E2>
    friend constexpr bool
    operator!=(unexpected &x, unexpected<E2> &y) noexcept(has_noexcept_not_equal_to_v<E, E2>) {
        return x.error() != y.error();
    }
...
template <typename E>
struct _expected_unwrap {
    using type = E;
};

template <typename E, E uniq_success>
struct _expected_unwrap<compressed_value<E, uniq_success>> {
    using type = E;
};

template <typename E>
using _expected_unwrap_t = typename _expected_unwrap<E>::type;
...
WJR_CONSTEXPR20 void reinit_expected(NewType &new_val, OldType &old_val, Args &&...args) {
    if constexpr (std::is_nothrow_constructible_v<NewType, Args...>) {
        ...
        std::destroy_at(std::addressof(old_val));
        wjr::construct_at(std::addressof(new_val), std::move(temp));
    } else {
...
struct expected_storage_base<T, compressed_value<E, uniq_success>, true> {
    constexpr expected_storage_base() noexcept(std::is_nothrow_default_constructible_v<T>)
        : m_val(), m_err(uniq_success) {}

    template <typename... Args>
    constexpr expected_storage_base(std::in_place_t, Args &&...args) noexcept(
        std::is_nothrow_constructible_v<T, Args...>)
        : m_val(std::forward<Args>(args)...), m_err(uniq_success) {}
    ...
    WJR_PURE constexpr bool has_value() const noexcept { return m_err == uniq_success; }
    constexpr void set_valid() noexcept { m_err = uniq_success; }
...
template <typename T, typename E, E uniq_success>
struct expected_storage_base<T, compressed_value<E, uniq_success>, false> {
    constexpr expected_storage_base() noexcept(std::is_nothrow_default_constructible_v<T>)
        : m_val(), m_err(uniq_success) {}
...
template <typename E, E uniq_success>
struct expected_storage_base<void, compressed_value<E, uniq_success>, true> {
    constexpr expected_storage_base() noexcept : m_err(uniq_success) {}

    constexpr expected_storage_base(std::in_place_t) noexcept : m_err(uniq_success) {}
...
struct expected_operations_base : expected_storage_base<T, E> {
    ...
    using error_type = _expected_unwrap_t<E>;
...
template <typename E>
struct _expected_storage_impl<void, E> {
    using error_type = _expected_unwrap_t<E>;
...
template <typename T, typename E>
using expected_storage = typename _expected_storage_impl<T, E>::type;

template <typename T, typename E>
struct _enable_expected_storage_impl {
...
template <typename E>
struct _enable_expected_storage_impl<void, E> {
    using type = enable_special_members_base<
...
template <typename T, typename E>
using enable_expected_storage = typename _enable_expected_storage_impl<T, E>::type;
...
using _expected_result = std::remove_cv_t<std::invoke_result_t<Func, Args...>>;

template <typename T, typename E>
class WJR_EMPTY_BASES expected
    : expected_detail::expected_storage<T, E>,
      expected_detail::enable_expected_storage<T, _expected_unwrap_t<E>> {
...
public:
    ...
    template <typename U, typename... Args>
    requires(std::is_constructible_v<error_type, std::initializer_list<U> &, Args...>)
    ...
    template <typename G>
    requires(std::is_constructible_v<error_type, G> && std::is_convertible_v<G, error_type>)
    ...
    WJR_CONSTEXPR20 expected(const expected<U, G> &other) : Mybase(enable_default_constructor) {
        if (other.has_value()) {
            this->construct_value(std::forward<const U &>(other.m_val));
        } else {
            this->construct_error(std::forward<const G &>(other.m_err));
    ...
    requires(std::is_constructible_v<T, const U &> &&
             ...
             expected_detail::expected_construct_with_v<T, error_type, U, G> &&
             !(std::is_convertible_v<const U &, T> && std::is_convertible_v<const G &, error_type>))
    WJR_CONSTEXPR20 explicit expected(const expected<U, G> &other)
        : Mybase(enable_default_constructor) {
        if (other.has_value()) {
            this->construct_value(std::forward<const U &>(other.m_val));
        } else {
    ...
    WJR_CONSTEXPR20 expected(expected<U, G> &&other) : Mybase(enable_default_constructor) {
        if (other.has_value()) {
            this->construct_value(std::forward<U>(other.m_val));
        } else {
            this->construct_error(std::forward<G>(other.m_err));
    ...
    requires(std::is_constructible_v<T, U> && std::is_constructible_v<error_type, G> &&
             expected_detail::expected_construct_with_v<T, error_type, U, G> &&
             !(std::is_convertible_v<U, T> && std::is_convertible_v<G, error_type>))
    WJR_CONSTEXPR20 explicit expected(expected<U, G> &&other) : Mybase(enable_default_constructor) {
        if (other.has_value()) {
            this->construct_value(std::forward<U>(other.m_val));
        } else {
            this->construct_error(std::forward<G>(other.m_err));
    ...
    requires(expected_detail::expected_construct_with_arg_v<T, error_type, U>)
    ...
    template <typename U>
    requires(!std::is_same_v<remove_cvref_t<U>, expected> &&
    ...
    template <typename Func, typename U = _expected_result<Func, T &>>
    ...
    template <typename Func, typename U = _expected_result<Func, const T &>>
    ...
    template <typename Func, typename U = _expected_result<Func, error_type &>>
    ...
    template <typename Func, typename U = _expected_result<Func, const error_type &>>
    ...
    template <typename Func, typename G = _expected_result<Func, error_type &>>
    ...
    template <typename Func, typename G = _expected_result<Func, const error_type &>>
...
private:
    static constexpr bool _is_nothrow_swappable =
...
public:
    ...
    requires(std::is_swappable_v<_T> && std::is_swappable_v<error_type> &&
    ...
    template <typename U>
    requires(expected_detail::is_not_expected<remove_cvref_t<U>>::value &&
             expected_detail::is_not_unexpected<remove_cvref_t<U>>::value)
...
class WJR_EMPTY_BASES expected<void, E>
    : expected_detail::expected_storage<void, E>,
      expected_detail::enable_expected_storage<void, _expected_unwrap_t<E>> {
...
public:
    ...
    template <typename Func, typename U = _expected_result<Func>>
    constexpr U and_then(Func &&func) & {
        if (has_value()) {
    ...
    template <typename Func, typename U = _expected_result<Func, const E &&>>
    constexpr U or_else(Func &&func) const && {
        if (WJR_UNLIKELY(!has_value())) {
            return std::invoke(std::forward<Func>(func), std::move(error()));
        }
