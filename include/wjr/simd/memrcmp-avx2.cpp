...
#define __WJR_AVX2_ONE_MEMRCMP(s0, s1)												\
	{																				\
		...
		if (z != __Mask) {															\
			return (s0) + _Mysize - wjr::countl_one(static_cast<uint32_t>(z)) / _Mycor;	\
		}																			\
	}
...
#define __WJR_AVX2_FOUR_MEMRCMP(s0, s1)														\
	{																						\
		...
		if(Z != __Mask){																	\
			...
			if(z0 != __Mask){																\
				return (s0) + _Mysize * 1 - wjr::countl_one(static_cast<uint32_t>(z0)) / _Mycor;	\
			}																				\
			auto z1 = traits::movemask_epi8(r1);											\
			if(z1 != __Mask){																\
				return (s0) + _Mysize * 2 - wjr::countl_one(static_cast<uint32_t>(z1)) / _Mycor;	\
			}																				\
			auto z2 = traits::movemask_epi8(r2);											\
			if(z2 != __Mask){																\
				return (s0) + _Mysize * 3 - wjr::countl_one(static_cast<uint32_t>(z2)) / _Mycor;	\
			}																				\
			auto z3 = traits::movemask_epi8(r3);											\
			if(z3 != __Mask){																\
				return (s0) + _Mysize * 4 - wjr::countl_one(static_cast<uint32_t>(z3)) / _Mycor;	\
			}																				\
		}																					\
	}
...
const uint8_t* avx2_memrcmp(const uint8_t* s0, const uint8_t* s1, size_t n) {
	...
	{
		__WJR_AVX2_ONE_MEMRCMP(s0 - _Mysize, s1 - _Mysize);
	}
	...
	{
		__WJR_AVX2_ONE_MEMRCMP(s0 - _Mysize * 2, s1 - _Mysize * 2);
	}
	...
	{
		__WJR_AVX2_ONE_MEMRCMP(s0 - _Mysize * 3, s1 - _Mysize * 3);
		__WJR_AVX2_ONE_MEMRCMP(s0 - _Mysize * 4, s1 - _Mysize * 4);
	}
