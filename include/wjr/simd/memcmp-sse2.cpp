...
#define __WJR_SSE2_ONE_MEMCMP(s0, s1)											                    \
	{																			                    \
		...
		if (z != __Mask) {														                    \
			return (s0) + wjr::countr_one(static_cast<uint16_t>(z)) / _Mycor;	                    \
		}																		                    \
	}

#define __WJR_SSE2_FOUR_MEMCMP(s0, s1)													            \
	{																					            \
		...
		if(Z != __Mask){																	        \
			...
			if(z0 != __Mask){																        \
				return (s0) + wjr::countr_one(static_cast<uint16_t>(z0)) / _Mycor;				    \
			}																				        \
			auto z1 = traits::movemask_epi8(r1);											        \
			if(z1 != __Mask){																        \
				return (s0) + _Mysize * 1 + wjr::countr_one(static_cast<uint16_t>(z1)) / _Mycor;	\
			}																				        \
			auto z2 = traits::movemask_epi8(r2);											        \
			if(z2 != __Mask){																        \
				return (s0) + _Mysize * 2 + wjr::countr_one(static_cast<uint16_t>(z2)) / _Mycor;	\
			}																				        \
			auto z3 = traits::movemask_epi8(r3);											        \
			if(z3 != __Mask){																        \
				return (s0) + _Mysize * 3 + wjr::countr_one(static_cast<uint16_t>(z3)) / _Mycor;	\
			}																				        \
		}																					        \
	}

const uint8_t* sse2_memcmp(const uint8_t* s0, const uint8_t* s1, size_t n) {
	...
	case 1: {
		...
		if (n) {
			__WJR_SSE2_ONE_MEMCMP(s0 - _Mysize, s1 - _Mysize);
		}
	...
	default: {
		...
		if (n) {
			s0 += n;
			s1 += n;
			__WJR_SSE2_FOUR_MEMCMP(s0 - _Mysize * 4, s1 - _Mysize * 4);
		}
		return s0;
	}
	}
