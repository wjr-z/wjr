...
#define __WJR_AVX2_ONE_MEMCMPEQ(s0, s1)											\
	{																			\
...
#define __WJR_AVX2_FOUR_MEMCMPEQ(s0, s1)													\
	{																						\
...
bool avx2_memcmpeq(const uint8_t* s0, const uint8_t* s1, size_t n) {
	...
	if (n < _Mysize) {
		...
		if (n >= threshold) {
			...
			return *(uint32_t*)(s0 + n - threshold) == *(uint32_t*)(s1 + n - threshold);
		}
		else {
			switch (n) {
			case 3: {
	...
	if (n <= _Mysize * 2) {
		...
		if (n != _Mysize) {
			s1 += n;
			__WJR_AVX2_ONE_MEMCMPEQ(s0 - _Mysize, s1 - _Mysize);
		}
		return true;
	}
	...
	if (n <= _Mysize * 4) {
		s0 += n;
		s1 += n;
		__WJR_AVX2_ONE_MEMCMPEQ(s0 - _Mysize * 2, s1 - _Mysize * 2);
		__WJR_AVX2_ONE_MEMCMPEQ(s0 - _Mysize, s1 - _Mysize);
		return true;
	}
	...
	{
		__WJR_AVX2_ONE_MEMCMPEQ(s0 + _Mysize * 2, s1 + _Mysize * 2);
		__WJR_AVX2_ONE_MEMCMPEQ(s0 + _Mysize * 3, s1 + _Mysize * 3);
	}

	if (n <= _Mysize * 8) {
		s0 += n;
		s1 += n;
		__WJR_AVX2_FOUR_MEMCMPEQ(s0 - _Mysize * 4, s1 - _Mysize * 4);
		return true;
	}
	...
	while (n >= _Mysize * 4) {
		__WJR_AVX2_FOUR_MEMCMPEQ(s0, s1);
		s0 += _Mysize * 4;
		s1 += _Mysize * 4;
		n -= _Mysize * 4;
	}
