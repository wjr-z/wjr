...
constexpr void push_back(forward_list_node *tail, forward_list_node *node) noexcept {
    WJR_ASSERT_ASSUME(tail->next == nullptr);
    tail->next = node;
}
...
constexpr void pop_front_uninit(forward_list_node *head,
                                forward_list_node *node) noexcept {
    WJR_ASSERT_ASSUME(head->next == node);
    head->next = node->next;
}

constexpr void pop_front_init(forward_list_node *head, forward_list_node *node) noexcept {
