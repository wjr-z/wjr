...
TEST_CASE("biginteger - construct") {
    {
        biginteger a;
...
TEST_CASE("biginteger - assignment") {
    {
        biginteger a;
        a = 0;
        biginteger b;
...
TEST_CASE("biginteger - sqr") {
    {
        ...
        for (int i = 0; i < T; ++i) {
            for (int j = 1; j < N; j += (j <= 180 ? 1 : j / 7)) {
                ...
                CHECK(std::equal(b.begin(), b.begin() + j * 2, c.begin()));
            }
        }
    }
...
TEST_CASE("biginteger - pow") {
    {
        biginteger a;
        pow(a, 0, 0);
        CHECK(a == 1u);
        pow(a, -14348907, 3);
        CHECK(a == biginteger("-2954312706550833698643"));
    }
#ifdef WJR_USE_GMP
    {
        biginteger a;
...
TEST_CASE("biginteger - mul_1") {
    ...
    for (int i = 0; i < T; ++i) {
        for (int j = 1; j < N; ++j) {
            ...
            CHECK(cf == cf2);
            CHECK(std::equal(b.begin(), b.begin() + j, c.begin()));
        }
    }
}

TEST_CASE("biginteger - addmul_1") {
...
TEST_CASE("biginteger - addlsh_n") {
    ...
    for (int i = 0; i < T; ++i) {
        for (int j = 1; j < N; ++j) {
            ...
            CHECK(cf == cf2);
            CHECK(std::equal(c.begin(), c.begin() + j, d.begin()));
    ...
    for (int i = 0; i < T; ++i) {
        for (int j = 1; j < N; j += (j <= 180 ? 1 : j / 7)) {
            for (int k = 1; k <= j; k += (k <= 180 ? 1 : k / 7)) {
                ...
                CHECK(std::equal(c.begin(), c.begin() + j + k, d.begin()));
            }
        }
    }
}

TEST_CASE("biginteger - mul_n") {
    ...
    for (int i = 0; i < T; ++i) {
        for (int j = 1; j < N; j += (j <= 180 ? 1 : j / 7)) {
            ...
            CHECK(std::equal(c.begin(), c.begin() + j * 2, d.begin()));
        }
    }
}
...
TEST_CASE("biginteger - div_qr_1") {
    ...
    auto check = [&](size_t n, uint64_t div, auto divc) {
        ...
        CHECK(cf == 0);
        CHECK(std::equal(a.begin(), a.begin() + n, c.begin()));
    };
...
TEST_CASE("biginteger - div_qr_s") {
    ...
    auto check = [&](size_t n, size_t m) {
        ...
        CHECK(cf == 0);
        CHECK(std::equal(a.begin(), a.begin() + n, d.begin()));
    };
    ...
    for (int t = 0; t < T; ++t) {
        for (unsigned base : {2, 4, 8, 16, 32, 10}) {
            for (int i = 1; i < N; ++i) {
                ...
                CHECK(std::string_view(b.data(), len) == std::string_view(c.data(), len2));
...
TEST_CASE("biginteger - from_chars") {
    const int T = 4;
    const int N = 8096;
    const int M = N;
    ...
    for (int t = 0; t < T; ++t) {
        for (int i = 1; i < M;) {
            for (auto base : {2, 8, 16, 10}) {
                ...
                CHECK(len == len2);
                CHECK(std::equal(a.begin(), a.begin() + len, b.begin()));
            }
...
TEST_CASE("biginteger - to_chars_extended") {
...
TEST_CASE("biginteger - stream_output") {
    ...
    // Test large numbers
    {
        ...
        CHECK(oss.str() == "1267650600228229401496703205375");
        oss.str("");
        oss.clear();
    }
...
// Test stream operator operator>>
TEST_CASE("biginteger - stream_input") {
    ...
    // Test stream already in error state
    {
        ...
        a = a_before;
        iss >> a;
        CHECK(iss.fail());    // Should remain in error state
        CHECK(a == a_before); // Value should be unchanged
        iss.clear();
    }
    ...
    // Test invalid input with different bases
    {
        iss.str("gg");
        iss >> std::hex >> a;
        CHECK(iss.fail()); // Should fail for invalid hex
        iss.clear();

        iss.str("999");
        iss >> std::oct >> a;
        CHECK(iss.fail()); // Should fail for invalid octal
        iss.clear();
...
TEST_CASE("biginteger - roundtrip_conversion") {
...
#define WJR_TEST_SUBC_1_I(inputs, c, c_in, outputs, ans)                                           \
    {                                                                                              \
        ...
        CHECK((subc_1(out, in, N, c, c_in) == ans));                                           \
        CHECK((memcmp(out, expect, sizeof(out)) == 0));                         \
        CHECK((subc_1(in, in, N, c, c_in) == ans));                                            \
        CHECK((memcmp(in, expect, sizeof(out)) == 0));                              \
    }
